...
async function findAll(req, res) {
    try {
        ...
        const responseData = rows.map(markaz => {
            const comments = markaz.comments || []; // comments mavjudligini tekshirish
            const totalStars = comments.reduce((sum, comment) => sum + (comment.star || 0), 0); // Yig‘indi
            const avgStar = comments.length > 0 ? totalStars / comments.length : 0; // O‘rtacha baho
        
            return {
                ...markaz.toJSON(),
                likeCount: markaz.likes ? markaz.likes.length : 0, // likes mavjudligini tekshirish
                star: avgStar, // O‘rtacha baho hisoblash
            };
        });
